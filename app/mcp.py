...
mcpapp = FastMCP(
    name="MSYS2Server", stateless_http=True, json_response=False, transport_security=custom_settings
)
...
class MCPRepository(BaseModel):
    ...
    pacman_url: str = Field(
        ...,
        description="A full URL to a location where the database, packages, and signatures for this repository can be found.",
    )
    src_url: str = Field(
        ...,
        description="Git source URL of the repository, where the PKGBUILD and other source files can be found.",
    )
...
class MCPBasePackage(BaseModel):
    ...
    packages: list[MCPPackage] = Field(
        default_factory=list, description="List of packages that belong to this base package"
    )
...
def msys2_search_base_packages(query: str, limit: int = 25) -> list[MCPBasePackage]:
    ...
    for src in find_packages(query, "pkg")[:limit]:
        ...
        for pkg in src.packages.values():
            pkgres.append(
                MCPPackage(
                    name=pkg.name, version=pkg.version, description=pkg.desc, repository=pkg.repo
                )
            )
